...
from ddpm import LinearNoiseScheduler, NoisePredictionDenoiser
...
# load VAE
vae = VariationalAutoencoder.load_model_from_file("vae_baseline.pt")
vae = vae.to(device)
vae.eval()
...
# extract latent representations for training data
with torch.no_grad():
...
def score_ldm_recon(x_test_flat, vae, scheduler, denoiser, device, noise_level = 500):
    """compute reconstruction error in latent space using the trained latent diffusion model"""
    denoiser.eval()
    video_scores = []
    for vid_feats in x_test_flat:
        with torch.no_grad():
            ...
            recon = vae.decode(latent_denoised)
...
def score_ldm_denoise_error(x_test_flat, vae, scheduler, denoiser, device, n_timesteps = 20):
...
for noise_level in [100, 250, 500, 750]:
    scores = score_ldm_recon(x_test_by_video, vae, scheduler_latent, denoiser_latent, device, noise_level=noise_level)
...
scores_denoise = score_ldm_denoise_error(x_test_by_video, vae, scheduler_latent, denoiser_latent, device)
